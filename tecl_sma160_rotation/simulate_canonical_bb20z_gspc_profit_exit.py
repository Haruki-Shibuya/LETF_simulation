...
DEFAULT_GSPC_OHLC_PATH = OUTPUT_DIR / "gspc_actual_ohlc_for_soxl_sma200_exit.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gspc-ohlc-path", type=Path, default=DEFAULT_GSPC_OHLC_PATH)
    ...
    parser.add_argument("--uvix-gspc-profit-exit-pct", type=float, default=0.1)
    ...
    parser.add_argument(
        "--output-stem",
        default="canonical_bb20z_gspc_profit_entry67p5_exit66p0_gamma0p1_low_rsi_tqqq_from_20100212",
    )
...
def main() -> None:
    ...
    gspc = read_by_date(args.gspc_ohlc_path)
    ...
    dates = sorted(
        ...
        if date >= args.backtest_start and date in ohlc and date in uvix and date in gspc
    )
    active_uvix = False
    ...
    for date in dates:
        ...
        gspc_open = as_float(index.get("GSPC_OPEN"))
        required = [
            rsi,
            bb20_z,
            tqqq_open,
        ...
        if active_uvix:
            ...
            gspc_profit_exit = gspc_open <= uvix_entry_gspc_open * (
                1.0 + args.uvix_gspc_profit_exit_pct / 100.0
            )
    ...
    metrics.update(
        {
            ...
            "uvix_gspc_profit_exit_pct": args.uvix_gspc_profit_exit_pct,
    ...
    print(
        "Canonical BB20Z + GSPC profit exit: "
