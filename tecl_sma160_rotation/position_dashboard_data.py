...
def build_position_dashboard_payload(mode: str = "latest", variant: str = DEFAULT_VARIANT) -> dict[str, Any]:
    ...
    return {
        ...
        "rules": {
            ...
            "高RSI UVIX終了条件": "翌日以降の始値で判断。①Implied RSI (14) <= 66.0 になった場合、または②翌日以降のGSPC始値が発動時始値×1.001以下になった場合にUVIXを終了する。②が成立する局面ではGSPCが発動時の水準付近まで戻っており、UVIXは概ね利益圏にある。さらなる下落を待って欲を出すのではなく、この時点でさっさと手仕舞うことがシミュレーション上のリターンを最大化するという知見を反映したルール。",
