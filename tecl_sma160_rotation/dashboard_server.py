...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/canonical_chart":
            ...
            return
        if parsed.path in {"", "/"}:
            self.redirect("/canonical-chart.html")
            return
    ...
    def respond_canonical_chart(self) -> None:
        try:
            body = json.dumps(build_canonical_chart_payload(), ensure_ascii=False).encode("utf-8")
...
def main() -> None:
    ...
    print(f"Dashboard: http://{display_host}:{args.port}/canonical-chart.html", flush=True)
